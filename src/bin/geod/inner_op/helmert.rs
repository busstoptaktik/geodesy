...
fn helmert_fwd(op: &Op, _prv: &dyn Provider, operands: &mut [CoordinateTuple]) -> usize {
    ...
    let fixed_t = op.params.boolean("fixed_t");
...
pub fn new(parameters: &RawParameters, _provider: &dyn Provider) -> Result<Op, Error> {
    ...
    let ROT = rotation_matrix(R.to_vec(), exact, position_vector);
    dbg!(ROT);
...
fn rotation_matrix(r: Vec<f64>, exact: bool, position_vector: bool) -> [[f64; 3]; 3] {
    ...
    // Destructuring assignments are still unstable - otherwise this if-block
