...
#[allow(dead_code)]
pub(super) fn ntv2_basegrid(buf: &[u8]) -> Result<BaseGrid, Error> {
...
impl Ntv2Grid {
    ...
    fn find_grid(&self, coord: &Coor4D, margin: f64) -> Option<(String, &BaseGrid)> {
        ...
        while let Some(grid_id) = queue.pop() {
            ...
            if current_grid.contains(coord, 1e-6, true) {
        ...
        if current_grid_id == "NONE" {
            ...
            for base_grid_id in self.lookup_table.get(&current_grid_id).unwrap() {
                if let Some(base_grid) = self.subgrids.get(base_grid_id) {
                    if base_grid.contains(coord, margin, true) {
...
impl Grid for Ntv2Grid {
    ...
    fn contains(&self, position: &Coor4D, margin: f64, _all_inclusive: bool) -> bool {
        self.find_grid(position, margin).is_some()
    }

    fn which_subgrid_contains(&self, coord: Coor4D, margin: f64) -> Option<String> {
        if let Some(id) = self.find_grid(&coord, margin) {
            return Some(id.0);
        }
        None
    }
...
mod tests {
    ...
    fn ntv2_grid() -> Result<(), Error> {
        ...
        assert!(ntv2_grid.contains(&barc, 0.5, true));
        assert!(!ntv2_grid.contains(&ldn, 0.5, true));
    ...
    fn ntv2_multi_subgrid_find_grid() -> Result<(), Error> {
        ...
        let grid_id = ntv2_grid
            .which_subgrid_contains(within_densified_grid, 1e-6)
            .unwrap();
        assert_eq!(grid_id, "5556");
        ...
        let grid_id = ntv2_grid
            .which_subgrid_contains(on_densified_upper_lat, 1e-6)
            .unwrap();
        assert_eq!(grid_id, "5458");
        let grid_id = basegrid
        ...
        assert_eq!(grid_id, "5458");
        let grid_id = ntv2_grid
            .which_subgrid_contains(on_densified_upper_lon, 1e-6)
            .unwrap();
        assert_eq!(grid_id, "5458");
        let grid_id = basegrid
        ...
        assert_eq!(grid_id, "5556");
        let grid_id = ntv2_grid
            .which_subgrid_contains(on_densified_lower_lat, 1e-6)
            .unwrap();
        assert_eq!(grid_id, "5556");
        let grid_id = basegrid
        ...
        assert_eq!(grid_id, "5556");
        let grid_id = ntv2_grid
            .which_subgrid_contains(on_densified_lower_lon, 1e-6)
            .unwrap();
        assert_eq!(grid_id, "5556");
        let grid_id = basegrid
        ...
        assert_eq!(grid_id, "5458");
        let grid_id = ntv2_grid
            .which_subgrid_contains(on_root_upper_lon, 1e-6)
            .unwrap();
        assert_eq!(grid_id, "5458");
        let grid_id = basegrid
        ...
        assert_eq!(grid_id, "5458");
        let grid_id = ntv2_grid
            .which_subgrid_contains(on_root_lower_lat, 1e-6)
            .unwrap();
        assert_eq!(grid_id, "5458");
        let grid_id = basegrid
        ...
        assert_eq!(grid_id, "5458");
        let grid_id = ntv2_grid
            .which_subgrid_contains(on_root_lower_lon, 1e-6)
            .unwrap();
        assert_eq!(grid_id, "5458");
        let grid_id = basegrid
        ...
        assert_eq!(grid_id, "5458");
        //let grid_id = ntv2_grid.which_subgrid_contains(on_root_upper_lat, 1e-6).unwrap();
        //assert_eq!(grid_id, "5458");
        let grid_id = basegrid
