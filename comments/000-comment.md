...
* **Coordinates and data flow pathways are four dimensioonal:** From end to end, data flow through RG along 4D pathways. Since all geodata capture today is either directly or indirectly based on GNSS, the coordinates are inherently four dimensional. And while much application software ignores this fact, embracing it is the only way to obtain sub-decimeter accuracy over time scales of just a few years.
